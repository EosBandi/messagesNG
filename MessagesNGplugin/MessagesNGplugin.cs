...
namespace MessagesNGPlugin
{
    ...
    public class MessagesNGPlugin : Plugin
    {
        ...
        private BlockingCollection<GCSMessage> localMessageQueue = new BlockingCollection<GCSMessage>();
		...
		//Loop is called in regular intervalls (set by loopratehz)
        {
            ...
            if (localMessageQueue.Count > 0) // Check if there are messages in the queue, otherwise the GetConsumingEnumerable.Take will block
            {
                while (localMessageQueue.Count > 0)
                {
                    var msg = localMessageQueue.Take();
                    mpMessagesControl.AddNewMessageWithoutRefresh(msg);
                }
                mpMessagesControl.UpdateLines();
            }
